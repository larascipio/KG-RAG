...
#         vectorize_data(t, embeddings_model, url, username, password_neo)
    ...
    # # from user prompt to query
    # prompt = "Which pink items are suitable for children?"
    # structured_query = define_query(prompt)
    ...
    #     input_variables=['similar_items'],
    #     template='Found {len(similar_items)} these products that might interest you: {similar_items}'
    # )
 
    # llm = AzureChatOpenAI(temperature=0, model="gpt-4o", api_version='2024-02-15-preview')
    # chain = prompt_template | llm | output_template
    ...
    # result = query_graph(structured_query)
 
    # final_response = chain.invoke({'prompt':prompt, 'similar_items':result})
 
    # Result
    # print(f"Found {len(result)} these products that might interest you: \n")
    # for r in result:
    ...
    system_prompt = CustomPromptTemplate(
    ...
    # llm_with_tools = llm.bind_tools([query_db, similarity_search])
   
    # LLM chain consisting of the LLM and a prompt
    # llm_chain = LLMChain(llm=llm, prompt=prompt)
    # llm_chain = prompt | llm
    # create_tool_calling_agent = (prompt, llm_with_tools)
    ...
    chain = system_prompt | llm | output_parser
 
 
    # custom_input = {'input': "I'm searching for pink shirts"}  
    user_prompt =  "Which pink items are suitable for children?"
 
    custom_input = {'input': user_prompt, 'intermediate_steps': []}
 
    # Running the LLM chain
    output = chain.invoke(custom_input)
